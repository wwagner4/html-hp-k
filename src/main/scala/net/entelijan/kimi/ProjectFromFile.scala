...
object ProjectFromFile {
  ...
  val inputDirName = System.getProperty("user.home") + "/work/work-html-hp-k/input"
  val inputFile = KimiUtil.inputFile
  val colCount = 26
  ...
  def projects: CreationResult[List[Project]] = {
    ...
    new CreationResult[List[Project]] {
      def report = new Report {
        def heading = Some("Created %d projects from %s" format(re.size, inputFile))
