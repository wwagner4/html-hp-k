...
case object Homepage {
  ...
  def build(): Unit = {
    ...
    if (!validateHompageAlphaResult.result) {
      ...
      val allReports: List[Report] =
        List(validateHompageAlphaResult.report, projects.report, buildReport)
      val rep = new Report {
  ...
  private def validateHompageAlpha(projects: List[Project]): CreationResult[Boolean] = {
    ...
    def projectsWithHpAlph(
        alph: Char,
        ps: List[Project],
        projectTitle: Project => ProjectTitle
    ): List[String] = ps match {
      case Nil => Nil
      case p :: rest =>
    ...
    val ger = 'A' to 'Z' map (char =>
      ProjectsForAlph(char, projectsWithHpAlph(char, projects, (p: Project) => p.title.ger))
    )
    val eng = 'A' to 'Z' map (char =>
      ProjectsForAlph(char, projectsWithHpAlph(char, projects, (p: Project) => p.title.eng))
    )
    ger.foreach(a => {
      if (a.projectsIDsWithHpAlph.size > 1) {
        ok = false
        infos ::= "'%s' is defined in more than one project for german. %s" format (a.alph, a.projectsIDsWithHpAlph
          .mkString(","))
      } else if (a.projectsIDsWithHpAlph.isEmpty) {
        infos ::= "Letter square: '%s' is not defined for german" format a.alph
      }
    })
    eng.foreach(a => {
      if (a.projectsIDsWithHpAlph.size > 1) {
    ...
    new CreationResult[Boolean] {
      ...
      def report: Report = new Report {
        def heading: Some[String] = if (ok) Some("Homepage Alpha Validation OK")
        else Some("ERROR: Homepage Alpha Validation. More than one Project for a Character")
